...
internal sealed class LiveSignalPipeline : ILiveSignalPipeline
{
    ...
    private readonly Subject<LiveSignalBatch> signalBatchesSubject = new();
    private PendingSignalBatch pendingSignalBatch = new();
    ...
    public LiveSignalPipeline(TimeSpan flushInterval, ILogger logger)
    {
        this.flushInterval = flushInterval;
        this.logger = logger;
    }

    public IObservable<LiveSignalBatch> SignalBatches => signalBatchesSubject.AsObservable();
    ...
    public void AppendTravelSamples(ReadOnlySpan<double> times, ReadOnlySpan<double> frontTravel, ReadOnlySpan<double> rearTravel)
    {
        ...
        lock (gate)
        {
            ...
            {
                pendingSignalBatch.TravelTimes.Add(times[i]);
                pendingSignalBatch.FrontTravel.Add(frontTravel[i]);
                pendingSignalBatch.RearTravel.Add(rearTravel[i]);
    ...
    public void AppendImuSamples(LiveImuLocation location, ReadOnlySpan<double> times, ReadOnlySpan<double> vibrationRms)
    {
        ...
        lock (gate)
        {
            ...
            if (!pendingSignalBatch.ImuTimes.TryGetValue(location, out var imuTimes))
            {
                imuTimes = new List<double>();
                pendingSignalBatch.ImuTimes[location] = imuTimes;
            }

            if (!pendingSignalBatch.ImuVibrationRms.TryGetValue(location, out var imuVibrationRms))
            {
                imuVibrationRms = new List<double>();
                pendingSignalBatch.ImuVibrationRms[location] = imuVibrationRms;
            }
    ...
    public void AppendFramePitchRollSamples(ReadOnlySpan<double> times, ReadOnlySpan<double> pitchDegrees, ReadOnlySpan<double> rollDegrees)
    {
        ...
        lock (gate)
        {
            ...
            {
                pendingSignalBatch.FramePitchRollTimes.Add(times[i]);
                pendingSignalBatch.FramePitchDegrees.Add(pitchDegrees[i]);
                pendingSignalBatch.FrameRollDegrees.Add(rollDegrees[i]);
            }
        }
    }
    ...
    public void Reset()
    {
        ...
        lock (gate)
        {
            ...
            pendingSignalBatch.Clear();
            recentTravelTimes.Clear();
            recentFrontTravel.Clear();
            recentRearTravel.Clear();
            signalRevision++;
            resetRevision = signalRevision;
        }

        signalBatchesSubject.OnNext(LiveSignalBatch.Empty with { Revision = resetRevision });
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        cts?.Dispose();
        signalBatchesSubject.OnCompleted();
        signalBatchesSubject.Dispose();
    }
    ...
    private async Task RunFlushLoopAsync(CancellationToken ct)
    {
        ...
        try
        {
            while (await timer.WaitForNextTickAsync(ct))
            {
                ...
                {
                    logger.Warning(ex, "Live projection flush failed");
                }
            }
        }
        catch (OperationCanceledException)
    ...
    private void FlushPendingSignalBatch()
    {
        ...
        int batchCount;
        long batchRevision;
        PendingSignalBatch batchToFlush;

        lock (gate)
        {
            if (!pendingSignalBatch.HasContent)
            {
                return;
            }

            batchToFlush = pendingSignalBatch;
            pendingSignalBatch = new PendingSignalBatch();
        ...
        try
        {
            ...
            signalBatchesSubject.OnNext(batch);
        }
        catch
        {
            lock (gate)
            {
                batchToFlush.AppendFrom(pendingSignalBatch);
                pendingSignalBatch = batchToFlush;
            }
    ...
    private sealed class PendingSignalBatch
    {
        ...
        public void AppendFrom(PendingSignalBatch other)
        {
