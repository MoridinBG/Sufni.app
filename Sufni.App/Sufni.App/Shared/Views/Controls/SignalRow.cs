...
public sealed class SignalRow : UserControl
{
    ...
    public static readonly StyledProperty<string?> TitleProperty =
        AvaloniaProperty.Register<SignalRow, string?>(nameof(Title));

    public static readonly StyledProperty<object?> TitleToolTipProperty =
        AvaloniaProperty.Register<SignalRow, object?>(nameof(TitleToolTip));

    public static readonly StyledProperty<string?> RowIdProperty =
        AvaloniaProperty.Register<SignalRow, string?>(nameof(RowId));

    public static readonly StyledProperty<SurfacePresentationState> PresentationStateProperty =
        AvaloniaProperty.Register<SignalRow, SurfacePresentationState>(
    ...
    public static readonly StyledProperty<object?> PlotContentProperty =
        AvaloniaProperty.Register<SignalRow, object?>(nameof(PlotContent));

    public static readonly StyledProperty<object?> PlaceholderContentProperty =
        AvaloniaProperty.Register<SignalRow, object?>(nameof(PlaceholderContent));

    public static readonly StyledProperty<IEnumerable<SignalRowAction>?> HeaderActionsProperty =
        AvaloniaProperty.Register<SignalRow, IEnumerable<SignalRowAction>?>(
            nameof(HeaderActions));

    public static readonly StyledProperty<bool> IsExpandedProperty =
        AvaloniaProperty.Register<SignalRow, bool>(nameof(IsExpanded), defaultValue: true);

    public static readonly StyledProperty<double> HeaderHeightProperty =
        AvaloniaProperty.Register<SignalRow, double>(nameof(HeaderHeight), defaultValue: 32);

    public static readonly StyledProperty<double> CollapsedHeaderHeightProperty =
        AvaloniaProperty.Register<SignalRow, double>(nameof(CollapsedHeaderHeight), defaultValue: 32);

    public static readonly StyledProperty<double> PreferredPlotHeightProperty =
        AvaloniaProperty.Register<SignalRow, double>(nameof(PreferredPlotHeight), defaultValue: 180);

    public static readonly StyledProperty<double> MinimumPlotHeightProperty =
        AvaloniaProperty.Register<SignalRow, double>(nameof(MinimumPlotHeight), defaultValue: 160);

    public static readonly StyledProperty<double> ChildRowGapProperty =
        AvaloniaProperty.Register<SignalRow, double>(nameof(ChildRowGap), defaultValue: 4);

    public static readonly StyledProperty<IBrush?> RowBackgroundProperty =
        AvaloniaProperty.Register<SignalRow, IBrush?>(nameof(RowBackground));

    public static readonly StyledProperty<IBrush?> HeaderBackgroundProperty =
        AvaloniaProperty.Register<SignalRow, IBrush?>(nameof(HeaderBackground));

    public static readonly StyledProperty<double> TitleLeftInsetProperty =
        AvaloniaProperty.Register<SignalRow, double>(nameof(TitleLeftInset));

    public static readonly StyledProperty<Color> PlotFigureBackgroundProperty =
        AvaloniaProperty.Register<SignalRow, Color>(
            nameof(PlotFigureBackground),
            defaultValue: SufniThemes.Fallback.SignalRow.Root.PlotFigure);

    public static readonly StyledProperty<Color> PlotDataBackgroundProperty =
        AvaloniaProperty.Register<SignalRow, Color>(
            nameof(PlotDataBackground),
            defaultValue: SufniThemes.Fallback.SignalRow.Root.PlotData);
    ...
    public IEnumerable<SignalRowAction>? HeaderActions
    {
    ...
    public AvaloniaList<SignalRow> ChildRows { get; } = [];
    ...
    public SignalRow()
    {
        ...
        headerActionsPresenter = new SignalRowActionsPresenter
        {
        ...
        ApplyRowDepthDefaults(currentTheme.SignalRow.Root);
        UpdateVisualState();
    }
    ...
    private void OnThemeVariantChanged()
    {
        ...
        headerConnectorBrush = currentTheme.SignalRow.Connector.ToBrush();

        var depthTheme = hierarchyDepth == 0
            ? currentTheme.SignalRow.Root
            : currentTheme.SignalRow.ByDepth(hierarchyDepth);
    ...
    private void OnChildRowsChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        ...
        {
            foreach (var oldRow in e.OldItems.OfType<SignalRow>())
            {
        ...
        {
            foreach (var newRow in e.NewItems.OfType<SignalRow>())
            {
    ...
    internal void ApplyHostedRowDefaults(int depth = 1)
    {
        ...
        ApplyRowDepthDefaults(currentTheme.SignalRow.ByDepth(depth));
        RebuildChildRows();
    }
    ...
    private void ApplyRowDepthDefaults(SufniSignalRowDepthTheme rowTheme)
    {
    ...
    internal bool HasDescendant(SignalRow candidate)
    {
    ...
    private double GetChildTopY(SignalRow child, SignalRow[] visibleChildren)
    {
        var offsetY = 0d;
        foreach (var row in visibleChildren)
        {
    ...
    private void OnHeaderPointerMoved(object? sender, PointerEventArgs args)
    {
        ...
        if (!isHeaderDragInProgress && HasExceededHeaderClickMovement(args))
        {
            ...
            this.FindAncestorOfType<SignalRowsRoot>()?.BeginRowDragFeedback(this);
        }

        if (isHeaderDragInProgress)
        {
            this.FindAncestorOfType<SignalRowsRoot>()?.UpdateRowDragFeedback(this, args);
            args.Handled = true;
        }
    }
    ...
    private void OnHeaderPointerReleased(object? sender, PointerReleasedEventArgs args)
    {
        ...
        if (isHeaderDragInProgress)
        {
            var root = this.FindAncestorOfType<SignalRowsRoot>();
    ...
    private void OnHeaderPointerCaptureLost()
    {
        if (isHeaderDragInProgress)
        {
            this.FindAncestorOfType<SignalRowsRoot>()?.EndRowDragFeedback(this);
        }
    ...
    {
        InvalidateMeasure();
        this.FindAncestorOfType<SignalRow>()?.InvalidateRowLayout();
