...
internal sealed class SignalRowsPanel : Panel
{
    ...
    private readonly Dictionary<SignalRow, AnimatedRowHeight> rowHeights = [];
    private readonly DispatcherTimer animationTimer;
    private double? viewportHeightOverride;

    public SignalRowsPanel()
    {
    ...
    protected override Size MeasureOverride(Size availableSize)
    {
        ...
        var autoGrowRows = new List<SignalRow>();
        ...
        foreach (var divider in Children.OfType<SignalBaseRowDivider>())
        {
    ...
    private IReadOnlyList<SignalRow> GetBaseRows()
        => Children.OfType<SignalRow>().ToArray();

    private IReadOnlyList<SignalRow> GetVisibleBaseRows()
    ...
    private static IReadOnlyList<SignalRow> GetRatioSizedRows(IReadOnlyList<SignalRow> visibleRows)
    {
    ...
    private static bool IsBoundaryDivider(SignalBaseRowDivider divider, IReadOnlyList<SignalRow> visibleRows)
    {
    ...
    private static bool IsResizableBoundaryTarget(SignalRow? row)
        => row is { IsExpanded: true } && row.GetVisiblePlotSlotCount() > 0;

    private double GetDisplayedHeight(SignalRow row)
        => rowHeights.TryGetValue(row, out var height)
            ? height.Current
            : row.AllocatedGroupHeight;

    private void SetDisplayedHeightTarget(SignalRow row, double target)
    {
    ...
    private void RemoveStaleAnimations(IReadOnlyList<SignalRow> baseRows)
    {
    ...
    private static int IndexOf(IReadOnlyList<SignalRow> rows, SignalRow targetRow)
    {
