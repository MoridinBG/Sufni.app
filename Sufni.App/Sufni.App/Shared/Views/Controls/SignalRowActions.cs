...
internal sealed class SignalRowActionsPresenter : UserControl
{
    ...
    private readonly List<SignalRowAction> subscribedActions = [];
    private IDisposable? themeVariantSubscription;
    private INotifyCollectionChanged? subscribedCollection;

    public static readonly StyledProperty<IEnumerable<SignalRowAction>?> ActionsProperty =
        AvaloniaProperty.Register<SignalRowActionsPresenter, IEnumerable<SignalRowAction>?>(
            nameof(Actions));

    public IEnumerable<SignalRowAction>? Actions
    {
        get => GetValue(ActionsProperty);
        set => SetValue(ActionsProperty, value);
    }

    public SignalRowActionsPresenter()
    {
    ...
    private void Rebuild()
    {
        ...
        foreach (var action in actions)
        {
            ...
            switch (action.Kind)
            {
                case SignalRowActionKind.Toggle:
                    toggleGroup.Children.Add(button);
                    break;
                case SignalRowActionKind.Execute:
    ...
    private IEnumerable<SignalRowAction> GetVisibleActions()
    {
        if (Actions is null)
        {
            return [];
    ...
    private void SubscribeToAction(SignalRowAction action)
    {
    ...
    private Button CreateActionButton(SignalRowAction action)
    {
    ...
    private static PathIcon CreateIcon(SignalRowAction action)
    {
        return new PathIcon
        {
    ...
    private void ApplyButtonColors(Button button, SignalRowAction action)
    {
        ...
        if (action.Kind == SignalRowActionKind.Toggle && action.IsChecked ||
    ...
    private static Color GetForegroundColor(SufniTheme theme, SignalRowAction action)
    {
        if (!action.IsEnabled)
        {
        ...
        {
            SignalRowActionTone.Accent => theme.Action.AccentPrimary,
            SignalRowActionTone.Danger => theme.Action.Danger,
            _ when action.Kind == SignalRowActionKind.Toggle && action.IsChecked => theme.Action.AccentPrimary,
