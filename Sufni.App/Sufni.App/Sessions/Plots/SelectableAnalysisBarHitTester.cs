...
internal sealed record SelectableAnalysisBarHit(
    Bar Bar,
    TelemetryRangeSelection.BinRange Bin,
    double Value,
    int Index);

internal sealed class SelectableAnalysisBarHitTester
{
    private readonly List<SelectableAnalysisBarHit> hits = [];

    public IReadOnlyList<SelectableAnalysisBarHit> Hits => hits;

    public void Clear() => hits.Clear();

    public void Register(Bar bar, TelemetryRangeSelection.BinRange bin, double value, int index)
    {
        hits.Add(new SelectableAnalysisBarHit(bar, bin, value, index));
    }

    public bool TryHit(double x, double y, out SelectableAnalysisBarHit hit)
    {
    ...
    public bool TryHitVerticalBarAtX(double x, out SelectableAnalysisBarHit hit)
    {
        foreach (var candidate in hits)
        {
            if (candidate.Bar.Orientation != Orientation.Vertical ||
...
internal static class SelectableAnalysisBarSelection
{
    public static void ApplyActiveAnalysisSelection(
        IEnumerable<SelectableAnalysisBarHit> hits,
