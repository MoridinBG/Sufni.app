...
internal sealed record SignalLayoutPreferenceRowState(
    string RowId,
    bool IsExpanded,
    double? HeightRatio,
    IReadOnlyList<SignalLayoutPreferenceRowState> Children);

// Pure preference-tree helper for recorded signal row ordering. It keeps invalid
// or stale row ids out of saved preferences and guards against cyclic moves.
internal static class SignalLayoutPreferenceTree
{
    public static SignalLayoutPreferences Normalize(
        SignalLayoutPreferences? preferences,
        IEnumerable<string> availableRowIds,
        SignalLayoutPreferences? defaults = null)
    {
        defaults ??= SignalLayoutPreferences.Default;
        ...
        return new SignalLayoutPreferences(rootRows.Select(ToPreferences).ToArray());
    }

    public static SignalLayoutPreferences Capture(IEnumerable<SignalLayoutPreferenceRowState> rows)
    {
        var used = new HashSet<string>(StringComparer.Ordinal);
        return new SignalLayoutPreferences(CaptureRows(rows, used).ToArray());
    }

    public static SignalLayoutPreferences MoveToRoot(
        SignalLayoutPreferences preferences,
        string rowId,
        int targetIndex)
    {
        ...
        return new SignalLayoutPreferences(roots.Select(ToPreferences).ToArray());
    }

    public static SignalLayoutPreferences MoveInto(
        SignalLayoutPreferences preferences,
        string rowId,
        string parentRowId,
        int targetIndex)
    ...
    public static SignalLayoutPreferences SetExpanded(
        SignalLayoutPreferences preferences,
        string rowId,
        bool isExpanded)
    {
    ...
    private static void AppendMissingDefaultRows(
        IEnumerable<SignalLayoutRowPreferences> defaults,
        ISet<string> available,
        ISet<string> used,
        IDictionary<string, RowNode> nodesById,
    ...
    private static IEnumerable<SignalLayoutRowPreferences> CaptureRows(
        IEnumerable<SignalLayoutPreferenceRowState> rows,
        ISet<string> used)
    {
        foreach (var row in rows)
        {
            ...
            yield return new SignalLayoutRowPreferences(
                row.RowId,
                row.IsExpanded,
    ...
    private static IEnumerable<RowNode> CloneRows(IEnumerable<SignalLayoutRowPreferences> rows)
    {
        var used = new HashSet<string>(StringComparer.Ordinal);
    ...
    private static IEnumerable<RowNode> CloneRows(
        IEnumerable<SignalLayoutRowPreferences> rows,
        ISet<string> used)
    {
        foreach (var row in rows)
        {
    ...
    private static SignalLayoutRowPreferences ToPreferences(RowNode node) =>
