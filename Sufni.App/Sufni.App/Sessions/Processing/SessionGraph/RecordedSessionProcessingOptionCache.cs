...
/// The recorded-session graph and domain query read it while evaluating staleness
...
/// and publishes <see cref="OptionChanged"/> so the graph can re-evaluate the
/// affected sessions (the preference-&gt;graph edge).
/// </summary>
public interface IRecordedSessionProcessingOptionCache
{
    ...
    // the graph's first sweep is gated on HydrateAsync completing instead.
