...
Sufni uses either classic linear sensors or it can also use rotary encoders
connected with linkages to measure movement. They are cheap, reasonably accurate,
and can reach pretty high sample rates. An additional benefit is that
on some frames they are easier to attach to rear shocks than linear sensors,
because they can fit into and around tigther spaces.
...
The user interface provides plots that help with setting spring rate, damping, and overall bike balance. GPX tracks can be uploaded and synchronized with the plots. Plots and map zoom is synced making it easy to see how the suspension behaved at specific sections of the trail.
...
- Stream raw data from the DAQ over Wi-Fi and record live sessions
