...
{
    [AvaloniaFact]
    public async Task RecordedSessionSignalsDesktopView_RendersToolbarContributions()
    {
        var workspace = new RecordedSessionSignalsWorkspaceStub(CreateMinimal());
        var command = new TestCommand();
        workspace.ExtensionSlots.SignalToolbarCommands.Add(new RecordedSessionToolbarCommandContribution(
            "extension",
            "toolbar-command",
            Order: 0,
    ...
    [AvaloniaFact]
    public async Task RecordedSessionSignalsDesktopView_RendersSignalRowExtensionContributions()
    {
        var workspace = new RecordedSessionSignalsWorkspaceStub(CreateMinimal());
        var hostedContent = new HostedSignalRowContent("Hosted row");
        var hostedRowTarget = RecordedSessionSignalRowTarget.Extension("extension", "hosted-row");
        var headerAction = new SignalRowAction
        {
            Id = "ExtensionVelocityAction",
            Kind = SignalRowActionKind.Execute,
        ...
        workspace.ExtensionSlots.SignalRowHeaderActions.Add(new RecordedSessionSignalRowActionContribution(
            "extension",
            "velocity-action",
            Order: 0,
            RecordedSessionSignalRowTarget.BuiltIn(RecordedSessionBuiltInSignalRow.Velocity),
            headerAction));
        workspace.ExtensionSlots.SignalPlotContextMenuActions.Add(new RecordedSessionSignalPlotContextMenuContribution(
            "extension",
            "travel-context",
            Order: 0,
            RecordedSessionBuiltInSignalRow.Travel,
            contextAction));
        workspace.ExtensionSlots.HostedSignalRows.Add(new RecordedSessionHostedSignalRowContribution(
            "extension",
            "hosted-row",
            Order: 0,
        ...
        workspace.ExtensionSlots.SignalTimeRangeOverlays.Add(new RecordedSessionTimeRangeOverlayContribution(
            "extension",
            "travel-range",
            Order: 0,
        ...
        var velocityRow = Assert.Single(
            mounted.View.GetVisualDescendants().OfType<SignalRow>(),
            row => row.RowId == SignalRowIds.Velocity);
        ...
        Assert.DoesNotContain(hostedRowTarget.StableKey, FlattenRowIds(graphRoot.CaptureSignalLayoutPreferences().Rows));
    ...
    private static T GetNamedVisual<T>(Control root, string name)
        where T : Control
    {
        var rowsView = Assert.Single(root.GetVisualDescendants().OfType<RecordedSignalRowsView>());
        var visual = rowsView.FindControl<T>(name);
        Assert.NotNull(visual);
        return visual!;
    }

    private static IEnumerable<string> FlattenRowIds(IEnumerable<SignalLayoutRowPreferences> rows)
    {
    ...
    private sealed record HostedSignalRowContent(string Text) : IRecordedSessionHostedSignalRowContributionViewModel;
        ...
        INotifyPropertyChanged
    {
        ...
        public SurfacePresentationState TravelSignalState => travelSignalState ?? CreateTravelState(TelemetryData);
        public SurfacePresentationState VelocitySignalState => velocitySignalState ?? TravelSignalState;
        public SurfacePresentationState ImuSignalState => imuSignalState ?? CreateImuState(TelemetryData);
