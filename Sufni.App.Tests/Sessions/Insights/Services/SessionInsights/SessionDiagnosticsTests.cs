...
public class SessionDiagnosticsTests
{
    ...
    [Fact]
    public void Run_EmitsFullSessionInsightsFinding_WhenNoRangeIsSelected()
    {
        var telemetry = CreateTelemetry(front: BuildSide(), rear: BuildSide());

        var report = Run(telemetry, CreateRequest(telemetry, range: null));

        var finding = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.FullSessionInsights);
        Assert.Equal(SessionInsightsCategory.DataQuality, finding.Category);
        Assert.Equal(SessionInsightsSeverity.Watch, finding.Severity);
        Assert.Equal(SessionInsightsConfidence.Low, finding.Confidence);
        Assert.Equal(
    ...
    public void Run_EmitsShortSelectedRangeFinding_WithRangeDuration()
    {
        ...
        var finding = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.ShortSelectedRange);
    ...
    public void Run_EmitsOnlyNoStrokeStatistics_WhenBothEndsLackStrokeData()
    {
        ...
        Assert.Equal(SessionInsightsFindingId.NoStrokeStatistics, finding.Id);
        Assert.Equal(SessionInsightsSeverity.Watch, finding.Severity);
    ...
    public void Run_EmitsOneEndMissingStrokeData_AndSkipsBalance_WhenOneEndIsMissing()
    {
        ...
        var finding = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.OneEndMissingStrokeData);
        Assert.Equal(SessionInsightsSeverity.Info, finding.Severity);
        Assert.DoesNotContain(report.Findings, finding => finding.Id == SessionInsightsFindingId.BalanceSlopesDiverge);
    ...
    public void Run_EmitsShallowTravelUse_WithTypedEvidenceAndOrderedAdjustments()
    {
        ...
        var finding = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.ShallowTravelUse);
        Assert.Equal(SessionInsightsCategory.TravelUse, finding.Category);
        Assert.Equal(SessionInsightsSeverity.Watch, finding.Severity);
        Assert.Equal(SessionInsightsConfidence.Medium, finding.Confidence);
    ...
    public void Run_SelectsHighSpeedCompressionAlternate_WhenHscBandDominates()
    {
        ...
        var report = Run(telemetry, CreateRequest(telemetry, SelectedRange, dampingPercentages: dampingPercentages));
    ...
    [Theory]
    [InlineData(4, SessionInsightsSeverity.Watch, AdjustmentComponent.AirPressure)]
    [InlineData(6, SessionInsightsSeverity.Action, AdjustmentComponent.Tokens)]
    public void Run_GradesBottomoutSeverity_AndReordersAdjustmentPriorities(
        int bottomouts,
        SessionInsightsSeverity expectedSeverity,
        AdjustmentComponent expectedPrimaryComponent)
    {
        ...
        var finding = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.RepeatedBottomouts);
    ...
    public void Run_EmitsDynamicSagMismatch_TowardTheDeeperEnd()
    {
        ...
        var finding = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.DynamicSagMismatch);
        Assert.Equal(SessionInsightsCategory.Balance, finding.Category);
        Assert.Equal(SessionInsightsSeverity.Watch, finding.Severity);
    ...
    public void Run_AttributesShallowSlowCompressionToPacking_AndSuppressesSideDamping()
    {
        ...
        var report = Run(telemetry, CreateRequest(telemetry, SelectedRange, profile: SessionInsightsTargetProfile.Enduro));

        var finding = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.ResistingImpacts);
        Assert.Equal(SessionInsightsCategory.Packing, finding.Category);
        Assert.Equal(SessionInsightsSeverity.Watch, finding.Severity);
        ...
        Assert.Contains(
            ...
            finding.Evidence);
        Assert.Contains(finding.Evidence, evidence => evidence.Measurement == DiagnosticMeasurement.DampingBandHsc);
        Assert.Equal(
            [
            ...
            finding.Adjustments);
        Assert.DoesNotContain(report.Findings, finding => finding.Category == SessionInsightsCategory.ForkDamping);
    }
    ...
    public void Run_FlagsSlowRebound_AgainstProfileReference()
    {
        ...
        var report = Run(telemetry, CreateRequest(telemetry, SelectedRange, profile: SessionInsightsTargetProfile.Trail));

        var finding = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.ReboundSlowForProfileContext);
        Assert.Equal(SessionInsightsCategory.ForkDamping, finding.Category);
        Assert.Equal(SessionInsightsSeverity.Watch, finding.Severity);
        var reboundP95 = GetEvidenceValue(finding, DiagnosticMeasurement.ReboundP95, SuspensionType.Front);
        Assert.InRange(reboundP95, 1.0, SessionDiagnostics.GetProfileReferences(SessionInsightsTargetProfile.Trail).Rebound.Low);
        Assert.Equal(
    ...
    public void Run_BalanceAdjustmentClosesDominantCompressionBand_WhenSpeedsAreHealthy()
    {
        ...
        var finding = Assert.Single(report.Findings, finding =>
            finding.Id == SessionInsightsFindingId.BalanceSlopesDiverge &&
    ...
    public void Run_LimitedBalanceContext_SuppressesBalanceAdjustments()
    {
        ...
        var diverging = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.BalanceSlopesDiverge);
        Assert.Empty(diverging.Adjustments);
        var context = Assert.Single(report.Findings, finding => finding.Id == SessionInsightsFindingId.BalanceContextLimited);
        Assert.Equal(SessionInsightsSeverity.Info, context.Severity);
        Assert.Contains(
    ...
    public void Run_EmitsVibrationContextPerInstrumentedSide()
    {
        ...
        var vibrationFindings = report.Findings
            .Where(finding => finding.Id == SessionInsightsFindingId.VibrationContext)
            .ToList();
        Assert.Equal(2, vibrationFindings.Count);
        Assert.All(vibrationFindings, finding =>
        {
            Assert.Equal(SessionInsightsCategory.Vibration, finding.Category);
            Assert.Equal(SessionInsightsSeverity.Info, finding.Severity);
            Assert.Empty(finding.Adjustments);
            Assert.Equal(
                [
    ...
    public void Run_FallsBackToVibrationNotice_WhenImuPairingIsNotComparable()
    {
        ...
        var finding = Assert.Single(report.Findings, finding => finding.Category == SessionInsightsCategory.Vibration);
        Assert.Equal(SessionInsightsFindingId.VibrationNotUsedForRecommendations, finding.Id);
        Assert.Equal(SessionInsightsSeverity.Info, finding.Severity);
