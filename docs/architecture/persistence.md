...
- `PatchSessionTrackAsync(id, points, gpsOffsetSeconds?)` — recomputes GPS distance/ascent/descent from the supplied projected points and the persisted `duration_seconds` column, then calls `UpdateSessionTrackAsync`, passing a GPS offset only when the caller is intentionally realigning the session-window GPS segment. It does not read or decode the processed BLOB just to patch the track. It then invalidates `session_cache` (like the BLOB write paths) so the next mobile load rebuilds the cached presentation from the new track and metrics.
