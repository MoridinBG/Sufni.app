...
The management protocol is a separate framed TCP request/response protocol from the [LIVE protocol](live-streaming.md#live-wire-protocol). LIVE and MGMT share the DAQ's single-client TCP port advertised over mDNS as `_sufni._tcp`, so only one live or management connection can be active against a given DAQ at a time. The app currently uses this MGMT v2 wire protocol only for DAQ announcements whose TXT `live_proto` is `2`; diagnostics management is disabled for v3 live snapshots because MGMT v3 bytes are not specified. The diagnostics tab also gates its Set Time / CONFIG management commands on the live client being disconnected (see [Disconnected v2-only management](live-streaming.md#design-decisions)); network import still attempts list/download actions directly for v2 network stores and reports any single-client conflict as a load or per-file import failure.

## Service Surface

`IDaqManagementService` (`Sufni.App/Sufni.App/Acquisition/Services/IDaqManagementService.cs`) is the public surface; `DaqManagementService` (`Sufni.App/Sufni.App/Acquisition/Services/Management/DaqManagementService.cs`) is the implementation registered as a singleton in `App.axaml.cs`.
...
Each top-level method opens a fresh TCP connection, runs one operation, and disposes the connection. `OpenSessionAsync(...)` is the exception: it returns an `IDaqManagementSession` (`Sufni.App/Sufni.App/Acquisition/Services/Management/IDaqManagementSession.cs`) that keeps a single connection open across `GetFileAsync`, `MarkSstUploadedAsync`, and `TrashFileAsync` calls and disposes it on `DisposeAsync()`. List, set-time, and replace-config remain one-shot. `ImportSessionsCoordinator` opens at most one session per network endpoint at the start of an import batch and attaches it to every `NetworkTelemetryFile` from that endpoint via `AttachSession(...)`, avoiding one TCP handshake per file when importing or trashing many files at once; `NetworkTelemetryFile` falls back to one-shot service calls when no session is attached.

Connection, per-frame I/O, and the `PUT_FILE_COMMIT` reply each have independent timeouts (defaults 5s / 5s / 15s). Connection and operation-validation failures surface as `DaqManagementException`; low-level malformed-frame parsing can still throw `FormatException` from `ManagementProtocolReader`. Protocol-level errors are returned as a typed `DaqManagementResult.Error` / `DaqGetFileResult.Error` / `DaqListDirectoryResult.Error` carrying a mapped `DaqManagementErrorCode` and a human-readable message.

## Wire Protocol

The protocol is little-endian, framed, request/response with a per-request `RequestId` that the device echoes. Wire types and helpers live in `Sufni.App/Sufni.App/Acquisition/Services/Management/`:
...
- **`NetworkTelemetryDataStore`** (`Sufni.App/Sufni.App/Acquisition/Models/NetworkTelemetryDataStore.cs`) calls `ListDirectoryAsync(host, port, DaqDirectoryId.Root)` from `GetFiles()`. Network import discovery filters `_sufni._tcp` announcements to TXT `live_proto=2` before creating these stores, so v3 DAQs are not probed with the v2 MGMT protocol. The returned `DaqRootDirectoryRecord.Files` is partitioned into `DaqSstFileRecord` (mapped into importable `NetworkTelemetryFile`s), `DaqMalformedSstFileRecord` (mapped into non-importable `NetworkTelemetryFile`s carrying the malformed message), and `DaqConfigFileRecord` (skipped — the import flow ignores CONFIG entries).
- **`NetworkTelemetryFile`** (`Sufni.App/Sufni.App/Acquisition/Models/NetworkTelemetryFile.cs`) implements the [`ITelemetryFile`](acquisition.md#interfaces) hooks against the management protocol: `ReadSourceAsync(...)` streams the recording through `GetFileAsync(..., DaqFileClass.RootSst, recordId, ...)` into a temporary `sufni-source-*.SST` file, reads the bytes into a `TelemetryFileSource`, and deletes the temp file; `OnImported()` calls `MarkSstUploadedAsync(recordId)` after a successful session save; `OnTrashed()` calls `TrashFileAsync(recordId)` for the deferred-delete branch of the import workflow. All three prefer the attached `IDaqManagementSession` when present and fall back to one-shot service calls otherwise. `DaqGetFileResult.Error` and `DaqManagementResult.Error` outcomes are rethrown as `DaqManagementException` so failures bubble up to `ImportSessionsCoordinator` and surface as per-file failures rather than terminating the batch.
- **`LiveDaqDetailViewModel`** (`Sufni.App/Sufni.App/LiveDaq/ViewModels/Editors/LiveDaqDetailViewModel.cs`) backs the shared/mobile and desktop diagnostics views for non-import management. It calls `SetTimeAsync` (Set Time button), `GetFileAsync(host, port, DaqFileClass.Config, 0, ...)` (Edit CONFIG — downloads CONFIG into a `MemoryStream` and parses it through `DaqConfigDocument`), and `ReplaceConfigAsync` (CONFIG editor save and Upload Config flow). All three are gated by `CanManage` — known endpoint, endpoint protocol v2, and the LIVE client in `Disconnected` state — and run through a single shared `CancellableOperation` so that starting one cancels any pending one.
...
CONFIG is the DAQ's persistent settings file, exchanged as raw bytes over `GetFileAsync` / `ReplaceConfigAsync` (`DaqFileClass.Config`, record id `0`). The service forwards bytes only — interpretation lives in `DaqConfigDocument` (`Sufni.App/Sufni.App/Acquisition/Services/Management/DaqConfigDocument.cs`):
