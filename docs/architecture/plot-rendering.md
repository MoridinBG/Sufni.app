...
`ImuPlot.FrameColor` is published as a static field from the theme so `LiveImuPlot` can reuse it. `TravelZonePalette` exports the theme-owned travel-zone ramp as hex strings for the `VelocityHistogramPlot` 10-zone stack, indices 1 / 4 / 8 of `VibrationThirdsPlot`, and the Avalonia `TravelPercentageLegend` chip strip.
