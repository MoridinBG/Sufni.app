...
The workspace view models expose the presentation states and data bindings used by the graph rows, while the view layer composes the reusable `TelemetryPlotsRoot` hierarchy for recorded and live editors. Row hierarchy drag/drop is handled by the Avalonia root/row controls and does not change plot models or workspace state. See [UI Architecture](ui.md) for the row composition and [Live DAQ Streaming](live-streaming.md) for the live transport, capture, and save lifecycle.
