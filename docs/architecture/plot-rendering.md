...
**Smoothing.** `TelemetryDisplaySmoothing` (`Sufni.App/Sufni.App/Plots/TelemetryDisplaySmoothing.cs`) applies a box-filter moving average over fixed time durations: `Off` -> no-op, `Light` -> 9 ms, `Strong` -> 31 ms. Regular sampled telemetry derives an odd sample window from the displayed sample step; explicit track samples use their X timestamps directly. Recordings with fewer than three samples skip smoothing. The strategy is intentionally simpler than the upstream Savitzky-Golay derivative used inside `Sufni.Telemetry`: this layer is purely cosmetic and never alters the underlying `TelemetryData` or `TrackPoint` data. The level is per-row (Travel / Velocity / IMU / Pitch-Roll / Speed / Elevation) in `SessionPlotPreferences`, persisted alongside the session so each row remembers the user's choice.
