...
│   └── Segments[] (FirstDenseIndex, FirstSourceIndex, StartSeconds, Travel[], Velocity[]), HasGaps
...
`ISessionProcessedTelemetryReader` is the decoded-telemetry read path for session detail loads and recorded-session extensions. It asks `ISessionRepository.GetSessionPsstPayloadMetadataAsync(...)` for the lightweight processed-payload metadata before loading the raw processed BLOB. While a `SessionDetailViewModel` is loaded it retains its session id; retained sessions reuse one decoded `TelemetryData` instance for the same `(sessionId, Updated, ProcessingFingerprintJson)` key, and a key change replaces the retained lazy decode. Unretained reads decode one-off. The view model releases the retention after recorded-session extension scopes are disposed on unload. Because the same `TelemetryData` instance can be shared by plots, mobile detail building, and extension readers, consumers must treat decoded telemetry as read-only.
