...
`TelemetryProcessingVersion.Current` is `5`. Version 5 currentizes processed BLOBs for the fixed-dt Savitzky-Golay path, parallel front/rear side processing, pooled preprocessing scratch, and the flattened segment serialization shape. Outputs stay numerically equivalent within the tolerated low-bit range, but the version bump marks every prior-version BLOB recomputable so each device regenerates on the current binary. It is a recompute-eligibility change only — there is no schema migration.
