...
- **Match before commit.** A sync pull download commits the new bytes (and overwrites the fingerprint + BLOB-derived metrics to match) only when the downloaded fingerprint equals the target. The commit goes through `SessionTelemetryWriter.SwapSessionPsstAsync`, which decodes the transferred BLOB once for validation/duration and summary metrics, writes no `updated` timestamp, and invalidates `session_cache`; the missing-BLOB load path uses the same swap writer after fetching `/session/data/{id}`, upserts the refreshed session snapshot, and then reloads telemetry through `ISessionProcessedTelemetryReader`. This fingerprint check is the integrity guard that replaces an option/fingerprint field inside the blob.
