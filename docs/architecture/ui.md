...
The desktop sidebar (`MainPagesDesktopView`) and the mobile menu drawer (`SidePanel`) both expose a sun/moon toggle bound to `MainPagesViewModel.ToggleThemeCommand`. The active icon is chosen by `ObjectConverters.Equal` against the `CurrentThemeMode` mirror property the view model keeps in sync with `IThemeService.ThemeChanged`. Cross-device sync persists the mode through `ThemePreferencesSyncData` carried inside `AppPreferencesSyncData` so the variant follows the user between paired devices.
